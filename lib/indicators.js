...
export function rollingSlope(bars, window = 10) {
  ...
  return closes.map((_, i) => {
    ...
    const y = closes.slice(i - window + 1, i + 1)  // include current bar
...
export function regressionRange(bars, atrVal) {
  ...
  // pred at index n (one bar beyond the last) = intentional: projects the range
  // for the NEXT bar, used as a forward-looking support/resistance estimate.
